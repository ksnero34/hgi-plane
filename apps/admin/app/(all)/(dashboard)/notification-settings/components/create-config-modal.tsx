      ...
      <Dialog as="div" className="relative z-50" onClose={onClose}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <CustomSelect
              value={selectedTemplate ? templates.find(t => t.id === selectedTemplate) : null}
              label={selectedTemplate ? templates.find(t => t.id === selectedTemplate)?.name || "직접 설정" : "직접 설정"}
              onChange={(option: any) => handleTemplateSelect(option?.id || "")}
              input
            >
              <CustomSelect.Option value={null}>직접 설정</CustomSelect.Option>
              {templates.map(template => (
                <CustomSelect.Option key={template.id} value={template}>

  ...
  return [
    {
      ...
      onSelect: async (data) => {
        ...
        // Workflow validation
        if (workspaceSlug && entityDetails?.project_id) {
          try {
            const validationResult = await validateTransition(workspaceSlug.toString(), entityDetails.project_id, {
              issue_id: entityDetails.id,
              from_state_id: entityDetails.state_id || "",
              to_state_id: stateId,
            });

...
class LegacyToRichFiltersConverter:
    ...
    def convert(self, legacy_filters: dict, strict: bool = False) -> Dict[str, Any]:
        ...
        for legacy_key, value in legacy_filters.items():
            ...
            if legacy_key == "custom_fields":
                if isinstance(value, dict):
                    # Convert custom_fields dict to customproperty_* filters
                    for field_id, field_values in value.items():
                        if field_values and len(field_values) > 0:
                            # Create dynamic filter key: customproperty_{field_id}__in
                            custom_prop_key = f"customproperty_{field_id}__in"
                            # Join values with comma for 'in' operator
                            rich_filters[custom_prop_key] = ",".join(str(v) for v in field_values)
                continue

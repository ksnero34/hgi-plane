...
class WorkflowTransitionSerializer(BaseSerializer):
    ...
    def to_representation(self, instance):
        """Add reviewer_ids to the representation for form population"""
        data = super().to_representation(instance)
        data['reviewer_ids'] = list(instance.reviewers.values_list("reviewer_id", flat=True))
        return data
    ...
    def create(self, validated_data):
        # Remove reviewer_ids from validated_data before creating the model instance
        reviewer_ids = validated_data.pop('reviewer_ids', [])
        instance = super().create(validated_data)
        return instance

    def update(self, instance, validated_data):
        # Remove reviewer_ids from validated_data before updating the model instance
        reviewer_ids = validated_data.pop('reviewer_ids', [])

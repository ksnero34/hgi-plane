...
export const CustomFieldProperties: React.FC<TCustomFieldProperties> = observer((props) => {
  ...
  const renderFieldValue = (field: TCustomField) => {
    ...
    switch (field.field_type) {
      case "text":
        return (
          <span className="text-sm text-custom-text-200">{fieldValue}</span>
        );
      ...
      default:
        return (
          <span className="text-sm text-custom-text-200">{String(fieldValue)}</span>
        );
    }
  };
      ...
      {customFields.map((field) => {
        ...
        return (
          <div key={field.id} className="flex items-center gap-2">
            ...
            <span className="text-sm text-custom-text-300 min-w-0 flex-shrink-0">
              {field.name}:
            </span>

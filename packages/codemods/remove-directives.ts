    ...
    if (path.node.directives) {
      ...
      path.node.directives.forEach((directive) => {
        if (
          ...
          typeof directive.value.value === "string" &&
          directivesToRemove.has(directive.value.value)
        ) {
    ...
    if (path.node.body) {
      ...
      path.node.body.forEach((node) => {
        ...
        if (j.ExpressionStatement.check(node)) {
          ...
          if (
            j.StringLiteral.check(expression) &&
            directivesToRemove.has(expression.value)
          ) {
            shouldRemove = true;
          } else if (
            j.Literal.check(expression) &&
